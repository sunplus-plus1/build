...
int get_partition_info_idx_by_file_name(const char *file_name_ptr)
{
	...
	for (i = 0; i < NUM_OF_PARTITION; i++) {
		if (strncmp(isp_info.file_header.partition_info[i].file_name, file_name_ptr, SIZE_FILE_NAME) == 0) {
			return i;
			// break;
		}
	}
...
int gen_script_main(char *file_name_isp_script, int nand_or_emmc)
{
	...
	if (nand_or_emmc == IDX_NAND) {
		...
		for (i = 0; i < NUM_OF_PARTITION; i++) {
			...
			fprintf(fd, "uuid=${uuid_gpt_%s};", isp_info.file_header.partition_info[i].file_name);
		}
	...
	for (i = 0; i < NUM_OF_PARTITION; i++) {
		...
		if (isp_info.file_header.partition_info[i].flags & FLAGS_BCH1K60) {
			fprintf(fd, "fatload $isp_if $isp_dev $isp_ram_addr /%s 0x%x 0x%x\n", isp_info.file_name_pack_image, isp_info.file_header.partition_info[i].file_size,
				isp_info.file_header.partition_info[i].file_offset);
...
#if !defined(REDUCE_MESSAGE)
					snprintf(cmd, sizeof(cmd), "mtdparts add nand0 ${isp_mtdpart_size}@${isp_nand_addr} %s && mtdparts && printenv mtdparts", isp_info.file_header.partition_info[i].file_name);
#else
					snprintf(cmd, sizeof(cmd), "mtdparts add nand0 ${isp_mtdpart_size}@${isp_nand_addr} %s && printenv mtdparts", isp_info.file_header.partition_info[i].file_name);
#endif
			...
			if (nand_or_emmc == IDX_NAND) {
				...
				while (file_size) {
					...
					fprintf(fd, "fatload $isp_if $isp_dev $isp_ram_addr /%s 0x%x 0x%x\n", isp_info.file_name_pack_image, size,
				...
				snprintf(cmd, sizeof(cmd), "mtdparts add nand0 ${isp_mtdpart_size}@${isp_nand_addr} %s", isp_info.file_header.partition_info[i].file_name);
#else /* defined(PARTITION_SIZE_BAD_BLOCK_DOES_NOT_COUNT) */
				fprintf(fd, "setenv isp_mtdpart_size 0x${isp_find_area_new_size}\n");
				fprintf(fd, "setenv isp_find_area_start 0x${isp_find_area_start}\n");
				snprintf(cmd, sizeof(cmd), "mtdparts add nand0 ${isp_mtdpart_size}@${isp_find_area_start} %s", isp_info.file_header.partition_info[i].file_name);
#endif /* PARTITION_SIZE_BAD_BLOCK_DOES_NOT_COUNT */
				fprintf(fd, "echo %s\n", cmd);
				fprintf(fd, "%s\n", cmd);
				fprintf(fd, "if test \"$?\" = 0 ; then\n");
				...
				size_programmed = 0;
				if (file_size == 0) {
					// fprintf(fd, "ispsp set_emmc_blk %s 0x%x\n", isp_info.file_header.partition_info[i].file_name, BYTE2BLOCK(size_programmed));
	...
	for (i = 0; i < NUM_OF_PARTITION; i++) {
		...
		int tflag = 0;
		if ((strcmp(isp_info.file_header.partition_info[i].file_name, "spsdk") == 0) && (tflag == 0)) {
			printf("++++++hy test code++++++\n");
...
int pack_image(int argc, char **argv)
{
	...
	fprintf(fd, "    fatload $isp_if $isp_dev $isp_ram_addr /%s 0x%x 0x%x\n", isp_info.file_name_pack_image, isp_script_size[IDX_NAND], file_offset_isp_script[IDX_NAND]);
	fprintf(fd, "elif test \"$isp_main_storage\" = emmc ; then\n");
#ifdef SUPPORT_MAIN_STORAGE_IS_EMMC
	fprintf(fd, "    fatload $isp_if $isp_dev $isp_ram_addr /%s 0x%x 0x%x\n", isp_info.file_name_pack_image, isp_script_size[IDX_EMMC], file_offset_isp_script[IDX_EMMC]);
#else
...
int extract4update(int argc, char **argv, int extract4update_src)
{
	...
	for (i = 0; i < NUM_OF_PARTITION; i++) {
		...
		while (file_size) {
			...
			fprintf(fd2, "elif test \"$isp_main_storage\" = emmc ; then\n");
#ifdef SUPPORT_MAIN_STORAGE_IS_EMMC
			// snprintf(cmd, sizeof(cmd), "ispsp set_emmc_blk %s 0x%x", file_header_extract4update.partition_info[i].file_name, BYTE2BLOCK(size_programmed));
			// fprintf(fd2, "    echo %s\n", cmd);
			// fprintf(fd2, "    %s\n", cmd);
		...
		while (file_size) {
			...
			fprintf(fd2, "elif test \"$isp_main_storage\" = emmc ; then\n");
#ifdef SUPPORT_MAIN_STORAGE_IS_EMMC
			// snprintf(cmd, sizeof(cmd), "ispsp set_emmc_blk %s 0x%x", file_header_extract4update.partition_info[i].file_name, BYTE2BLOCK(size_verified));
			// fprintf(fd2, "    echo %s\n", cmd);
			// fprintf(fd2, "    %s\n", cmd);
	...
	// fprintf(fd2, "echo verifying %s ...\n", file_header.partition_info[0].file_name);
	// snprintf(cmd, sizeof(cmd), "nand read.bblk $isp_ram_addr %s 0x%x", file_header.partition_info[0].file_name, file_header.partition_info[0].file_size);
	// fprintf(fd2, "echo %s\n", cmd);
	// fprintf(fd2, "%s\n", cmd);
	//
